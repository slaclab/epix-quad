...
char tixelPixelThreshold(int pixel, int pixThreshold) {
      ...
      while (!stop) {
         ...
         if ( dread_->next(event_)) {
            ...
            if (dsize_==1168+1152) {
               ...
               string fileName = "/u1/mkwiatko/Tixel_data.bin";
               ...
               for (int x = HEADER_SIZE+1, i=1; x < event_->size() - FOOTER_SIZE; x++, i++) {
                  ...
                  if (PRINT_FIRST_PIXELS) {
                     ...
                     //printf("%c", tixelPixelThreshold(((event_->data()[x]&0x0000ffff)), PIXEL_THRESHOLD));
                     printf("%c", tixelPixelThreshold(((event_->data()[x]&0xffff0000)>>16), PIXEL_THRESHOLD)); 
