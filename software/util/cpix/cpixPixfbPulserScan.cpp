...
int main (int argc, char **argv) {
   ...
   try {
      ...
      printf("Pixel feedback changed to %d\n", pixFb);
      value = epix.device("digFpga",0)->device("CpixPAsic",0)->readSingle("Config11");
      epix.device("digFpga",0)->device("CpixPAsic",0)->writeSingle("Config11", (value&0xffffffC7)|((pixFb&0x7)<<3));
            ...
            if (dsize_ >= 1168) {
               ...
               if ((event_->data()[HEADER_SIZE]&0xf0)!=0) {
                  ...
                  if (cntAevent >= EVENTS_PER_FRAME-1 && cntAframe <= FRAMES_PER_FILE-1) {
                     ...
                     cntAframe++;
                     // save the file
                     ostringstream cnvPfb;
                     ostringstream cnvMPul;
                     cnvPfb << pixFb;
                     cnvMPul << pulser;
                     string fileName = "/u1/mkwiatko/CPIX_" + string(timeString) + "_cntA_PixFb_" + cnvPfb.str() + "_Pulser_" + cnvMPul.str() + ".bin";
               ...
               //Frame with cntB was received
                  ...
                  if (cntBevent >= EVENTS_PER_FRAME-1 && cntBframe <= FRAMES_PER_FILE-1) {
                     ...
                     cnvPfb << pixFb;
                     cnvMPul << pulser;
                     string fileName = "/u1/mkwiatko/CPIX_" + string(timeString) + "_cntB_PixFb_" + cnvPfb.str() + "_Pulser_" + cnvMPul.str() + ".bin";
                     frameFile.open (fileName.c_str(), ios::out | ios::binary | ios::app);
                     frameFile.write ((char*)event_->data(), dsize_*4);
                     frameFile.close();
                  }
               ...
               if (cntAframe > FRAMES_PER_FILE-1 && cntBframe > FRAMES_PER_FILE-1) {
                  ...
                  if (pulser < PULSER_STOP && pulser+PULSER_STEP <= 1023) {
                     ...
                     if (pixFb < PIXFB_STOP && pixFb+PIXFB_STEP <= 7) {
