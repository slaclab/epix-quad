...
for SDclk in range(0, 16):
    for SDrst in range(0, 16):
        filename = fileFolders[0][0] + fileNameRoot + 'SDclk_' + str(SDclk) + 'SDrst_' + str(SDrst) + FileNameRun
        ...
        while ((len(file_header) > 0) and ((numberOfFrames < MAX_NUMBER_OF_FRAMES_PER_BATCH)
               or (MAX_NUMBER_OF_FRAMES_PER_BATCH == -1))):
            try:
                ...
                if ((GET_SERIAL_OR_TS_DATA == True) and (file_header[0] == PAYLOAD_SERIAL_FRAME)):
                    # (frame size splited by four to read 32 bit
                    newPayload = np.fromfile(f, dtype='uint32', count=payloadSize)
                    if (numberOfFrames == 0):
        ...
        if(SAVEHDF5):
            print("Saving Hdf5")
            index_h5 = 'hrAdc_%d' % (SDclk * 16 + SDrst)
...
if PLOT_SET_HISTOGRAM:
    ...
    for i in range(0, imgDesc.shape[0]):
        ...
        #   h, b = np.histogram(darkSub[i,x,y], np.arange(-nbins/2,nbins/2+1))
        #    h, b = np.histogram(np.average(darkSub[i,:,5]), np.arange(-nbins/2,nbins/2+1))
        h, b = np.histogram(np.average(imgDesc[i, np.arange(0, 32, 2), standAloneADCPlot]), np.arange(
            centralValue_even - nbins / 2, centralValue_even + nbins / 2 + 1))
        n_even = n_even + h
        h, b = np.histogram(np.average(imgDesc[i, np.arange(1, 32, 2), standAloneADCPlot]), np.arange(
            centralValue_odd - nbins / 2, centralValue_odd + nbins / 2 + 1))
        n_odd = n_odd + h
