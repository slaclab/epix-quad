...
//#define NCOL 48
#define NCH 16
#define NUMWORDS ((NROW+1)*NCOL*NCH/2 + 11)
#define NHEADERS 8
...
bool AccumulateEvent(ifstream &fin, unsigned int ch, float *pedestals, TTree *tree, bool advance, bool first = false);
TH1 *CalcFFT(TGraph *graph);
bool ReadTpsAndEnv(ifstream &fin, TTree *tree);

//void ProcessData(char *filename, char *ped_filename = NULL, bool visualize = true, char *out_root = NULL) {
void ProcessData(char *filename, char *out_root = NULL) {
   char *ped_filename = NULL;
   ...
   if (!fin_tuple) {
      cout << "Couldn't open second: " << filename << endl;
      return;
   }
   ifstream fin_ped;
   ...
   fin_tuple.seekg(ios_base::beg);

   unsigned int chs[4] = {0,4,8,12};
   //unsigned int chs[4] = {2,8,0,1};
   int bins = 1000;  //41
   float low = 8000; //-20
   float high = 11000; //20
   ...
   tree->Branch("event", NULL , "event/I");
   tree->Branch(  "col", NULL , "col/I");
   tree->Branch(  "row", NULL , "row/I");
   tree->Branch(   "ch", NULL , "ch/I");
   tree->Branch(  "adc", NULL , "adc/F");
//   tree->Branch(  "tps", NULL , "tps/F");
//   tree->Branch(  "env", NULL , "env/F");
   ...
   while(fin && fin_tuple) {
      bool fail = false;
      fail = AccumulateEvent(fin_tuple, chs[0], pedestalsU1, tree, false,first);
      first = false;
...
float* CalculatePedestals(ifstream &fin, unsigned int ch, int nevents) {
   ...
   for (int i = 0; i < nevents; ++i) {
      ...
      if (size != NUMWORDS) {
         ...
         cout << "\t size was " << size << " but expected " << NUMWORDS << endl;
      }
      fin.read( (char *) event_buffer, NUMWORDS*sizeof(unsigned int) );
      for (int row = 0; row < NROW; ++row) {
         for (int col = 0; col < NCOL; ++col) {
            ...
            float value = (float) adc_buffer[row*NCOL*4*2+channel_map[ch]*NCOL+temp_col];
...
bool ReadTpsAndEnv(ifstream &fin, TTree *tree) {
   ...
   fin.read( (char *) event_buffer, NUMWORDS*sizeof(unsigned int) );
   unsigned short int *env_buffer = (unsigned short int *) &(event_buffer[NHEADERS+NROW*NCOL*NCH/2 + NROW*NCOL*NCH/4]);
   unsigned short int *tps_buffer = (unsigned short int *) &(event_buffer[NHEADERS+(NROW+1)*NCOL*NCH/2]);
   for (int i = 0; i < 8; ++i) {
      cout << env_buffer[i] << " ";
   }
   for (int i = 0; i < 4; ++i) {
...
bool AccumulateEvent(ifstream &fin, unsigned int ch, float *pedestals, TTree *tree, bool advance, bool first) {
   int pos = fin.tellg();
...
*/
   ...
   tree->SetBranchAddress(   "ch", &temp_ch);
   tree->SetBranchAddress(  "adc", &value  );
   ...
   static unsigned int firstSeqCount = event_buffer[2];
   if (first) {
      lastAcqCount = event_buffer[1];
      lastSeqCount = event_buffer[2];
      firstSeqCount = event_buffer[2];
   }
   ...
   event = seqCount - firstSeqCount;
   unsigned short int *adc_buffer = (unsigned short int *) &(event_buffer[NHEADERS]);

   /* Testing tps and env readout */
   unsigned short int *env_buffer = (unsigned short int *) &(adc_buffer[NROW*NCOL*NCH + NCOL*NCH/2]);
   unsigned short int *tps_buffer = (unsigned short int *) &(adc_buffer[(NROW+1)*NCOL*NCH]);
   for (int i = 0; i < 8; ++i) {
      cout << env_buffer[i] << " ";
   }
   for (int i = 0; i < 4; ++i) {
   ...
   for (row = 0; row < NROW; ++row) {
      for (col = 0; col < NCOL; ++col) {
         ...
         value = (float) adc_buffer[row*NCOL*NCH+channel_map[ch]*NCOL+temp_col];
         value -= pedestals[row*NCOL+col];
