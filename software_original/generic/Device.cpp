...
//      ofstream fout("epix_reglist.txt",std::fstream::app);
//      fout <<  "0x" << setw(2) << setfill('0') << hex << (reg->address() >> 24);
//      fout << " 0x" << setw(8) << setfill('0') << hex << (reg->address());
//      fout << " 0x" << setw(8) << setfill('0') << hex << (reg->get());
//      fout << endl;
//      fout.close();
//   }
}
...
// Helpful methods for reading and writing registers

void Device::regReadInt ( string nameReg, string nameVar, uint bit, uint mask) {
   Register * r;   
...
void Device::regReadInt ( string name, uint bit, uint mask ){
   regReadInt(name,name,bit,mask);
}

void Device::regReadIntDec ( string nameReg, string nameVar, uint bit, uint mask) { 
   Register * r;
   r = getRegister(nameReg);
   readRegister(r);
   getVariable(nameVar)->setIntDec(r->get(bit, mask)); 
}

void Device::regReadIntDec ( string name, uint bit, uint mask ){
   regReadInt(name,name,bit,mask);
}

void Device::regWrite ( string nameReg, string nameVar, bool force, uint bit, uint mask ) {
   Register * r;
   r = getRegister(nameReg);
...
void Device::regWrite ( string name, bool force, uint bit, uint mask ){
