...
def main(argv):
   ...
   for defaultG in ("True","False"):
      for defaultGA in ("True","False"):
         ...
         if ( not(pixelG == "True" and pixelGA == "True") ):
            continue;
         ...
         for highResSetting in ("True","False"):
            ...
            for baseRow in range(0,48):
               ...
               #Set the matrix into a default state
               WriteMatrix(pythonDaq, defaultTest, defaultMask, defaultG, defaultGA)
               #but always mask the first row
               WriteRow(pythonDaq,0,"False","True",pixelG,pixelGA);
               #write the current row of interest
               if (baseRow != 0):
                  WriteRow(pythonDaq,baseRow,pixelTest,pixelMask,pixelG,pixelGA)
               resSetting = "lr"
